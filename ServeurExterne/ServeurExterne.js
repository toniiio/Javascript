...
function RechercherLienServeur()
	{
		ajaxGet("https://oc-jswebsrv.herokuapp.com/api/liens",function(reponse){
		var tableauxLiens = JSON.parse(reponse);
		tableauxLiens.forEach(function(extraire){
		var liens = {
		url : extraire.url ,
		auteur : extraire.auteur ,
		titre : extraire.titre 
		};
		var lienServeurElt = creerElementLien(liens);
		contenuElt.insertBefore(lienServeurElt,contenuElt.firstChild);	
	});
	});
	}
	// rechargement de la page web affiche toujours le nouveau lien puisque celui-ci est sauvegardé sur le serveur
	if(window.addEventListener) 
	window.onload = RechercherLienServeur;
...
ajouterLienElt.addEventListener("click", function () {
    ...
    formAjoutElt.addEventListener("submit", function (e) {
		...
		//Envoi les liens sur le serveur
		ajaxPost(
		"https://oc-jswebsrv.herokuapp.com/api/lien",
		lien,function(reponse){},true);
